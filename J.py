...
st.title("📊 Dashboard Perbandingan Harga Saham")
st.write("Dashboard ini membandingkan harga penutupan saham hari ini dengan 3 bulan yang lalu.")

# Kolom input untuk pengguna memasukkan kode saham
tickers_input = st.text_input(
    "Masukkan kode saham (pisahkan dengan koma, misal: BBCA.JK, BBRI.JK, AAPL):", 
...
# Tombol untuk mengeksekusi pencarian
    ...
    with st.spinner("Mengambil data saham..."):
        ...
        for ticker in tickers:
            try:
                ...
                # Mengambil data historis dari 3 bulan lalu hingga sekarang
                hist = stock.history(start=start_date.strftime('%Y-%m-%d'), end=end_date.strftime('%Y-%m-%d'))
                
                if not hist.empty:
                    harga_lama = hist['Close'].iloc[0]      # Harga hari pertama di periode 3 bln lalu
                    harga_sekarang = hist['Close'].iloc[-1] # Harga penutupan terakhir
                    
                    # Rumus menghitung persentase perbedaan
                    persentase = ((harga_sekarang - harga_lama) / harga_lama) * 100
                    
                    data.append({
                        "Kode Saham": ticker,
                        "Harga 3 Bln Lalu": round(harga_lama, 2),
                        "Harga Sekarang": round(harga_sekarang, 2),
                        "Perubahan (%)": round(persentase, 2)
                    })
            except Exception as e:
                st.error(f"Gagal mengambil data untuk saham: {ticker}")
                
        # Jika data berhasil dikumpulkan, tampilkan dalam tabel
        if data:
            df = pd.DataFrame(data)
            
            # Fungsi untuk memberi warna merah jika minus, hijau jika plus
            def warnai_persentase(val):
                color = 'red' if val < 0 else 'green'
                return f'color: {color}; font-weight: bold;'
                
            # Tampilkan dataframe di Streamlit
            st.dataframe(
                df.style.map(warnai_persentase, subset=['Perubahan (%)']), 
