...
if current_batch:
    with st.spinner(f"⏳ Loading..."):
        ...
        if not data.empty:
            ...
            fig, axes = plt.subplots(rows_needed, COLS, figsize=(20, CHART_HEIGHT * rows_needed), dpi=400)
            ...
            for i, ticker in enumerate(current_batch):
                ...
                try:
                    ...
                    if df_stock.empty or df_stock['Close'].isnull().all():
                        ax.text(0.5, 0.5, "❌ No Data", ha='center', va='center', fontsize=6, color='red')
                    ...
                    ax.text(0.5, 0.5, f"⚠️ Error", ha='center', va='center', fontsize=6, color='orange')
...
# --- 4. FOOTER ---
st.markdown("<div style='text-align: center; color: gray; font-size: 10px; margin-top: 10px;'>📈 IHSG Chart Generator | Yahoo Finance</div>", unsafe_allow_html=True)
