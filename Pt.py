...
st.set_page_config(page_title="Dividend Yield Tracker", layout="wide")
...
st.title("📊 Smart Dividend Yield Dashboard")
st.write(f"Update Terakhir: {datetime.now().strftime('%d %B %Y')}")

# ==========================================
# 2. FUNGSI AMBIL DATA (DENGAN FILTER ERROR)
# ==========================================
@st.cache_data(ttl=3600)
def get_clean_dividend_data(tickers):
    data_list = []
    ...
    for t in tickers:
        try:
            ...
            if not div_history.empty:
                # 1. Hitung Total Dividen 1 Tahun Terakhir (Trailing 12 Months)
                one_year_ago = datetime.now() - timedelta(days=365)
                # Filter dividen setahun terakhir
                div_1y = div_history[div_history.index.tz_localize(None) > one_year_ago]
                total_div_1y = div_1y.sum()
                
                # 2. Hitung Yield secara Manual (Lebih Akurat)
                if current_price > 0:
                    calc_yield = (total_div_1y / current_price) * 100
                else:
                    calc_yield = 0
                
                # 3. Data Dividen Terakhir
            ...
            # Label Status untuk mendeteksi data aneh
            status = "Normal"
            if calc_yield > 40: 
                status = "⚠️ Outlier (Cek RTI)"
            elif calc_yield == 0:
                status = "No Div (1Y)"
            ...
            data_list.append({
                ...
                "Total Div 1Y": round(total_div_1y, 2) if 'total_div_1y' in locals() else 0,
                "Last Div Amount": last_amount,
...
# ==========================================
with st.spinner('Menghitung yield berdasarkan histori 1 tahun...'):
    df = get_clean_dividend_data(MY_WATCHLIST)

if not df.empty:
    # Baris Atas: Ringkasan
    c1, c2, c3, c4 = st.columns(4)
    avg_y = df[df["Yield 1Y (%)"] > 0]["Yield 1Y (%)"].mean()
    high_y = len(df[df["Yield 1Y (%)"] > 7])
    
    c1.metric("Avg Yield (Non-Zero)", f"{avg_y:.2f}%")
    c2.metric("High Yield (>7%)", f"{high_y} Saham")
    c3.metric("Ticker Termahal", df.loc[df['Price'].idxmax()]['Ticker'])
    c4.metric("Total Watchlist", len(df))

    st.divider()

    # Baris Tengah: Tabel Data
    st.subheader("📋 Detail Data Dividen")
    
    # Styling Tabel
    def highlight_status(val):
        if "Outlier" in str(val): return 'background-color: #ffcccc'
        if "Normal" in str(val): return 'color: #2e7d32'
        return ''

    st.dataframe(
        df.style.applymap(highlight_status, subset=['Status'])
        .background_gradient(cmap='YlGn', subset=['Yield 1Y (%)'])
        .format({"Price": "{:,.0f}", "Last Div Amount": "{:,.2f}", "Total Div 1Y": "{:,.2f}"}),
    ...
    with col_right:
        st.subheader("💡 Tips")
        st.info("""
        - **Yield 1Y (%)**: Dihitung dari total dividen yang cair dalam 365 hari terakhir dibagi harga saat ini.
        - **Status Outlier**: Muncul jika yield > 40%. Biasanya terjadi karena dividen spesial atau kesalahan data stock split.
        - **Last Ex-Date**: Tanggal terakhir kumulatif dividen (Cum Date) biasanya 1 hari kerja sebelum Ex-Date.
        """)
else:
    st.error("Data tidak ditemukan. Coba periksa koneksi internet atau format Ticker saham kamu.")
