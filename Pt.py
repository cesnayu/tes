...
st.set_page_config(page_title="Multi-Dividend Tracker", layout="wide")

st.title("🗓️ Dividend Multi-Payment Tracker")
st.write(f"Menampilkan semua riwayat dividen dalam 1 tahun terakhir (Update: {datetime.now().strftime('%d/%m/%Y')})")

# ==========================================
# 2. LOGIKA PENGAMBILAN DATA
# ==========================================
@st.cache_data(ttl=3600)
def get_multi_dividend_data(tickers):
    results = []
    ...
    for t in tickers:
        try:
            ...
            # Ambil seluruh histori dividen
            div_history = stock.dividends
            
            if not div_history.empty:
                # Filter hanya 1 tahun terakhir (365 hari)
                one_year_ago = datetime.now() - timedelta(days=365)
                div_1y = div_history[div_history.index.tz_localize(None) > one_year_ago]
                
                # Hitung total nominal dividen dalam setahun
                total_nominal = div_1y.sum()
                
                # Hitung berapa kali bagi dividen dalam setahun
                frequency = len(div_1y)
                
                # AMBIL SEMUA TANGGAL & NOMINAL (Contoh: "15/01 (Rp100), 20/06 (Rp250)")
                details = []
                for date, amount in div_1y.items():
                    date_str = date.strftime('%d %b %Y')
                    details.append(f"{date_str} ({amount:,.0f})")
                
                all_payments_str = " | ".join(details)
                
                # Hitung Yield
                calc_yield = (total_nominal / current_price) * 100 if current_price > 0 else 0
            else:
                total_nominal = 0
                frequency = 0
                all_payments_str = "Tidak ada pembagian"
                calc_yield = 0

            results.append({
                "Ticker": t,
                "Harga": current_price,
                "Yield 1Thn (%)": round(calc_yield, 2),
                "Total Nominal (1Thn)": total_nominal,
                "Frekuensi": f"{frequency}x",
                "Rincian Tanggal & Nominal (Ex-Date)": all_payments_str,
                "Status": "Normal" if calc_yield < 40 else "⚠️ Cek Manual (Outlier)"
            })
        except:
            continue
...
# ==========================================
with st.spinner('Menyisir data pembayaran dividen...'):
    df_final = get_multi_dividend_data(MY_WATCHLIST)

if not df_final.empty:
    # Summary Cards
    col1, col2, col3 = st.columns(3)
    col1.metric("Total Emiten", len(df_final))
    col2.metric("Rata-rata Yield", f"{df_final['Yield 1Thn (%)'].mean():.2f}%")
    col3.metric("Dividen Terbesar", df_final.loc[df_final['Yield 1Thn (%)'].idxmax()]['Ticker'])

    st.divider()

    # Tabel Utama
    st.subheader("📋 Jadwal & Riwayat Dividen Setahun Terakhir")
    st.dataframe(
        df_final.style.background_gradient(cmap='YlGn', subset=['Yield 1Thn (%)'])
        .format({"Harga": "{:,.0f}", "Total Nominal (1Thn)": "{:,.2f}"}),
        use_container_width=True
    )

    # Visualisasi Frekuensi
    st.subheader("📊 Frekuensi Pembagian Dividen")
    st.bar_chart(df_final.set_index('Ticker')['Yield 1Thn (%)'])
    
    st.info("""
    **Cara Membaca Kolom Rincian:**
    - Format: **Tanggal (Nominal)**.
    - Jika muncul lebih dari satu tanggal (dipisahkan oleh tanda '|'), artinya saham tersebut membagi dividen beberapa kali (Interim & Final atau Kuartalan).
    - Tanggal yang digunakan adalah **Ex-Dividend Date**.
    """)
else:
    st.error("Data tidak tersedia. Coba cek kembali ticker saham kamu.")
