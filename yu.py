...
def get_data_safe(tickers, period="3mo"):
    ...
    try:
        # Mengunduh data. auto_adjust=True membantu agar kolom 'Close' konsisten
...
def make_chart(df, ticker_name):
    ...
    pct_change = ((last_val - start_val) / start_val) * 100
    df_pct = ((df['Close'] - start_val) / start_val) * 100
    
    clr = '#00C805' if last_val >= start_val else '#FF333A'
    
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=df.index, y=df_pct, mode='lines', 
    ...
    fig.update_layout(
        title=dict(
            text=f"<b>{ticker_name.replace('.JK','')}</b><br><span style='font-size:11px;'>Rp{last_val:,.0f} ({pct_change:+.2f}%)</span>",
            x=0.5, y=0.85, font=dict(size=14, color=clr)
        ),
        margin=dict(l=5, r=5, t=50, b=5), height=220, showlegend=False,
        yaxis=dict(ticksuffix="%", showgrid=True, gridcolor='rgba(128,128,128,0.1)', tickfont=dict(size=9)),
...
# --- 6. MAIN UI ---
st.title("📋 Saham IHSG (Growth %)")

c1, c2, c3 = st.columns([1, 1, 1])
with c1: tf = st.selectbox("Rentang Waktu", ["5d", "1mo", "3mo", "6mo", "1y", "2y"], 2)
with c2: min_p = st.number_input("Min Harga", 0, value=0)
with c3: max_p = st.number_input("Max Harga", 0, value=1000000)
...
if b1.button("⬅️") and st.session_state.page > 1:
    ...
    st.rerun()
b2.markdown(f"<div style='text-align:center;'>Hal {st.session_state.page} / {total_pages}</div>", unsafe_allow_html=True)
...
data_all = get_data_safe(batch, period=tf)
...
if data_all is not None and not data_all.empty:
    ...
    for t in batch:
        try:
            # Ambil sub-dataframe untuk ticker t
            if len(batch) > 1:
                # Pola penanganan multi-index yfinance
                dft = data_all[t].dropna()
            else:
                dft = data_all.dropna()
            ...
            # Filter Harga
            p_now = dft['Close'].iloc[-1]
            if not (min_p <= p_now <= max_p): continue
            
            # Buat Chart
            fig = make_chart(dft, t)
            if fig:
                with cols[idx % 4]:
    ...
    if idx == 0:
        st.info("Tidak ada data yang cocok dengan kriteria harga.")
else:
    st.error("Gagal menarik data dari server Yahoo Finance.")
