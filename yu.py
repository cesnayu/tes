...
st.markdown("""
<style>
    .stPlotlyChart { border: 1px solid rgba(128,128,128,0.1); border-radius: 10px; }
</style>
...
# --- 5. FUNGSI DATA DENGAN RETRY ---
@st.cache_data(ttl=600, show_spinner=False)
def get_data_stable(tickers, period="3mo"):
    if not tickers: return pd.DataFrame()
    
    # Percobaan maksimal 2 kali
...
def chart_grid_pct(df, ticker):
    ...
    fig.update_layout(
        title=dict(
            text=f"<b>{ticker.replace('.JK','')}</b><br><span style='font-size:11px;'>Rp{last_price:,.0f} ({last_pct:+.2f}%)</span>", 
...
# --- 7. MAIN UI ---
st.title("📋 Market Observation (Growth %)")

# Filter Controls
c1, c2, c3 = st.columns([1, 1, 1])
with c1: tf = st.selectbox("Rentang Waktu", ["5d", "1mo", "3mo", "6mo", "1y", "2y", "5y"], 2)
with c2: min_p = st.number_input("Min Harga (Rp)", 0, value=0, step=50)
with c3: max_p = st.number_input("Max Harga (Rp)", 0, value=500000, step=500)
...
if b1.button("⬅️ Prev") and st.session_state.page > 1: 
    st.session_state.page -= 1
    st.rerun()
b2.markdown(f"<div style='text-align:center; font-weight:bold; padding-top:10px;'>Halaman {st.session_state.page} / {total_pages}</div>", unsafe_allow_html=True)
if b3.button("Next ➡️") and st.session_state.page < total_pages: 
    st.session_state.page += 1
    st.rerun()
...
if batch:
    with st.spinner(f"Menarik data batch {st.session_state.page}..."):
        df_all = get_data_stable(batch, period=tf)
    
    if not df_all.empty:
        # Cek apakah multi-index (jika download > 1 ticker biasanya multi-index)
        ...
        for t in batch:
            try:
                # Ambil dataframe khusus ticker t
                if is_multi:
                ...
                # Filter Harga
                curr_price = dft['Close'].iloc[-1]
                if not (min_p <= curr_price <= max_p): continue
        ...
        if grid_idx == 0:
            st.info("Tidak ada saham yang sesuai kriteria harga di halaman ini.")
    else:
        st.error("Gagal menarik data. Yahoo Finance mungkin sedang sibuk atau koneksi terputus. Coba klik 'Next' lalu balik lagi ke 'Prev'.")
    
    # Paksa pembersihan memori
    gc.collect()

st.divider()
st.caption("Data diperbarui via yfinance API. Sumbu Y menunjukkan % kenaikan/penurunan dari harga pembuka periode yang dipilih.")
