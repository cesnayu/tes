...
st.set_page_config(page_title="Dashboard Saham OHLC Return", layout="wide")

# --- 1. FITUR CACHING & FETCH REAL DATA (OHLC) ---
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_real_stock_data(tickers, time_range):
    # Mapping rentang waktu
    ...
    for ticker in tickers:
        try:
            # Mengambil data OHLC lengkap
            stock_data = yf.Ticker(ticker).history(period=period)
            
            if not stock_data.empty:
                # Membuat DataFrame bersih dengan OHLC
                df = pd.DataFrame({
                ...
                stocks.append({
                    'name': ticker.upper(),
                    'data': df,
                    'min': df['Low'].min(), # Min dari Low terendah
                    'max': df['High'].max() # Max dari High tertinggi
                })
...
# --- 2. KOMPONEN GRAFIK (KOLOM + GARIS HIGH-LOW + DARK MODE) ---
def draw_chart(stock):
    df = stock['data'].copy()
    
    # --- KALKULASI PERSENTASE RELATIF TERHADAP OPEN ---
    
    # 1. Body Kolom: Open ke Close (Intraday Return)
    df['Body_%'] = (df['Close'] - df['Open']) / df['Open'] * 100
    
    # 2. Garis Atas (High Wick): Open ke High
    df['High_Rel_%'] = (df['High'] - df['Open']) / df['Open'] * 100
    
    # 3. Garis Bawah (Low Wick): Open ke Low
    df['Low_Rel_%'] = (df['Low'] - df['Open']) / df['Open'] * 100
    
    # Menentukan Warna Kolom (Body)
    # Hijau jika Close >= Open, Merah jika Close < Open
    colors = ['#10b981' if val >= 0 else '#ef4444' for val in df['Body_%']]
    
    # Warna Garis High-Low (Wicks) - Gunakan EMAS agar kontras di latar hitam
    wick_color = '#FFD700' 
    
    fig = go.Figure()
    
    # --- TRACE 1: GARIS HIGH-LOW (Diletakkan di belakang kolom) ---
    # Kita menggunakan Scatter mode 'lines' untuk menggambar garis vertikal tunggal per hari
    wick_x = []
    wick_y = []
    # Membuat segmen garis vertikal [Low, High, None] untuk setiap tanggal
    ...
    # --- TRACE 2: KOLOM RETURN (Body: Open ke Close) ---
    fig.add_trace(go.Bar(
        x=df['Tanggal'], 
        y=df['Body_%'],
        marker_color=colors,
        name='Open-to-Close Return',
        # Tooltip interaktif menampilkan OHLC asli
        hovertemplate=(
            '<b>Tanggal: %{x}</b><br>' +
            'Return (O-C): %{y:.2f}%<br>' +
            '<span style="color:gray;">--------------------</span><br>' +
            'Open: Rp %{customdata[0]:,.0f}<br>' +
            'High: Rp %{customdata[1]:,.0f}<br>' +
            'Low: Rp %{customdata[2]:,.0f}<br>' +
            'Close: Rp %{customdata[3]:,.0f}<extra></extra>'
        ),
        # Mengirim data OHLC asli ke tooltip
        customdata=df[['Open', 'High', 'Low', 'Close']].values 
    ))
    
    # Konfigurasi Tema Dark Mode yang Dipertegas
    fig.update_layout(
        template="plotly_dark", 
        title={
            'text': f"<b>{stock['name']}</b><br><span style='font-size: 12px; color: #ef4444;'>Min Periode: {stock['min']:,.0f}</span> | <span style='font-size: 12px; color: #10b981;'>Max Periode: {stock['max']:,.0f}</span>",
            'y':0.9, 'x':0.05, 'xanchor': 'left', 'yanchor': 'top'
        },
        margin=dict(l=20, r=20, t=65, b=20),
        height=280, # Sedikit lebih tinggi untuk menampung wick
        xaxis_title="",
        yaxis_title="Return vs Open (%)",
        paper_bgcolor="black", # Latar belakang benar-benar hitam
        plot_bgcolor="black",
        showlegend=False, # Sembunyikan legenda agar bersih
    )
    
    # Grid abu-abu sangat gelap
    fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='#222222')
    # Garis 0% berwarna putih tipis sebagai referensi harga Open
    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='#222222', zeroline=True, zerolinewidth=1, zerolinecolor='#555555')
    
    return fig

# --- 3. UI DASHBOARD ---
st.title("📊 Dashboard Saham Intraday Return (OHLC)")
