...
# Konfigurasi Halaman (Mengaktifkan mode wide)
st.set_page_config(page_title="Dashboard Saham Real-Time", layout="wide")

# --- 1. FITUR CACHING & FETCH REAL DATA ---
# Mengambil data asli dari Yahoo Finance. Cache disimpan 1 jam.
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_real_stock_data(tickers, time_range):
    # Mapping rentang waktu ke format yang dipahami yfinance
    # 1w (1 minggu) ~ 5 hari kerja bursa, 1m (1 bulan) ~ 1mo, 3m (3 bulan) ~ 3mo
    ...
    for ticker in tickers:
        try:
            ...
            if not stock_data.empty:
                # Membuat DataFrame yang bersih
                df = pd.DataFrame({
                    'Tanggal': stock_data.index.date, # Hanya ambil tanggal (tanpa jam)
                    'Harga': stock_data['Close'].values # Menggunakan harga penutupan (Close)
                })
            ...
            # Jika saham tidak ditemukan atau error, lewati saja
            pass 
            
    return stocks

# --- 2. KOMPONEN GRAFIK (DARK MODE) ---
def draw_chart(stock):
    df = stock['data'].copy()
    
    # Menghitung Return Harian (%)
    df['Return_%'] = df['Harga'].pct_change() * 100
    df['Return_%'] = df['Return_%'].fillna(0) 
    
    # Menentukan Warna (Hijau jika Naik/Tetap, Merah jika Turun)
    ...
    # Konfigurasi Tema Dark Mode (Hitam & Putih)
    fig.update_layout(
        template="plotly_dark", # TEMA OTOMATIS HITAM, TEKS PUTIH
        title={
            'text': f"<b>{stock['name']}</b><br><span style='font-size: 12px; color: #ef4444;'>Min: {stock['min']:,.0f}</span> | <span style='font-size: 12px; color: #10b981;'>Max: {stock['max']:,.0f}</span>",
        ...
        xaxis_title="",
        yaxis_title="Return (%)",
        # Membuat latar belakang benar-benar hitam/transparan menyatu dengan tema dark
        paper_bgcolor="rgba(0,0,0,0)", 
        plot_bgcolor="rgba(0,0,0,0)"
    )
    
    # Grid abu-abu gelap agar tidak terlalu mencolok di background hitam
    fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='#333333')
    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='#333333', zeroline=True, zerolinewidth=2, zerolinecolor='white')
    
    return fig

# --- 3. UI DASHBOARD ---
st.title("📈 Dashboard Saham Real-Time")

# Input Kode Saham dari User (Pisahkan dengan koma)
# Tambahkan .JK untuk saham Indonesia (contoh: BBCA.JK)
...
user_input = st.text_area("✍️ Masukkan Kode Saham (Pisahkan dengan koma):", default_stocks)

# Memproses input user menjadi list
ticker_list = [ticker.strip().upper() for ticker in user_input.split(',')]
ticker_list = [t for t in ticker_list if t] # Hapus item kosong

# Perhitungan Paginasi (Maksimal 20 saham per halaman)
...
# Memastikan halaman saat ini tidak melebihi total halaman
...
if total_stocks == 0:
    ...
    # Memotong list saham sesuai halaman saat ini (20 per halaman)
    start_idx = (st.session_state.page - 1) * 20
    end_idx = start_idx + 20
    current_page_tickers = ticker_list[start_idx:end_idx]

    # Load data hanya untuk 20 saham di halaman ini
    with st.spinner(f'Mengambil data asli untuk {len(current_page_tickers)} saham...'):
        stocks_data = fetch_real_stock_data(current_page_tickers, time_range)

    if not stocks_data:
        st.error("Gagal mengambil data. Pastikan kode saham benar (Gunakan .JK untuk saham Indonesia, misal: BBCA.JK).")
    else:
        # Menampilkan Grafik dalam format 2 Kolom
        for i in range(0, len(stocks_data), 2):
            cols = st.columns(2)
            with cols[0]:
                # theme=None agar Streamlit mengikuti template "plotly_dark" yang kita set
