...
# --- LIST SAHAM (Bisa tambah banyak tanpa lemot) ---
...
def get_bulk_data(tickers):
    try:
        # Download semua sekaligus (1 request = Cepat)
            ...
            for i in range(0, len(u_tickers), 3):
                ...
                for j in range(3):
                    ...
                    if idx < len(u_tickers):
                        ...
                        with cols[j]:
                            try:
                                # Ambil data per ticker dari bulk
                                if len(u_tickers) > 1:
                                ...
                                if not t_data.empty:
                                    ...
                                    st.warning(f"Data {t} kosong")
                            except Exception:
                                st.error(f"Error pada {t}")
...
with tab2:
    ...
    if analysis_list:
        ...
        st.info("Gagal memproses data watchlist. Klik refresh.")
