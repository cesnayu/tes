...
# --- 2. HEADER & NAVIGATION (ATAS) ---
st.title("📈 Top IHSG")

# Hitung total untuk pagination
total_stocks = len(tickers)

# Layout kompak untuk kontrol atas
col_nav1, col_nav2, col_nav3, col_nav4 = st.columns([1, 1, 1, 3])

with col_nav1:
    STOCKS_PER_PAGE = st.selectbox("Saham/Hal", options=[25, 50, 100, 200], index=1, label_visibility="collapsed")
    total_pages = math.ceil(total_stocks / STOCKS_PER_PAGE)

with col_nav2:
    page = st.number_input("Hal", min_value=1, max_value=total_pages, value=1, label_visibility="collapsed")

with col_nav3:
    st.markdown(f"<div style='padding-top: 10px; font-size: 14px;'><b>{page}/{total_pages}</b> | Total: {total_stocks}</div>", unsafe_allow_html=True)

with col_nav4:
    st.markdown(f"<div style='padding-top: 10px; font-size: 14px; text-align: right;'>Saham {((page-1)*STOCKS_PER_PAGE)+1}-{min(page*STOCKS_PER_PAGE, total_stocks)}</div>", unsafe_allow_html=True)

# --- 3. PENGATURAN CHART (ATAS - KOMPAK) ---
with st.expander("⚙️ Pengaturan Chart", expanded=False):
    col_set1, col_set2, col_set3, col_set4, col_set5, col_set6 = st.columns(6)
    
    with col_set1:
        period_options = {"5 Hari": "5d", "1 Bulan": "1mo", "6 Bulan": "6mo", "1 Tahun": "1y", "3 Tahun": "3y", "5 Tahun": "5y", "10 Tahun": "10y", "Maksimal": "max"}
        selected_period_label = st.selectbox("Periode", options=list(period_options.keys()), index=2)
        PERIOD = period_options[selected_period_label]
    
    with col_set2:
        MA_WINDOW = st.selectbox("MA (hari)", options=[5, 10, 20, 50, 100, 200], index=2)
    
    with col_set3:
        COLS = st.selectbox("Kolom", options=[3, 4, 5, 6], index=2)
    
    with col_set4:
        CHART_HEIGHT = st.selectbox("Tinggi", options=[2, 3, 4, 5], index=1)
    
    with col_set5:
        SHOW_MA = st.checkbox("Tampil MA", value=True)
    
    with col_set6:
        SHOW_GRID = st.checkbox("Grid", value=True)

# Tentukan saham yang ditampilkan
...
# --- 4. DOWNLOAD & PLOTTING ---
if current_batch:
    with st.spinner(f"⏳ Mengambil data {selected_period_label}..."):
        ...
        if not data.empty:
            ...
            fig, axes = plt.subplots(rows_needed, COLS, figsize=(20, CHART_HEIGHT * rows_needed))
            ...
            for i, ticker in enumerate(current_batch):
                ...
                try:
                    ...
                    if df_stock.empty or df_stock['Close'].isnull().all():
                        ...
                        ax.set_title(ticker.replace('.JK', ''), fontsize=8, color='red')
                        ax.axis('off')
                        continue
                    ...
                    if SHOW_MA and len(close) >= MA_WINDOW:
                        ...
                        ax.plot(ma.index, ma, color='orange', linewidth=0.8, label=f'MA{MA_WINDOW}', alpha=0.8)

                    title = f"{ticker.replace('.JK', '')} ({pct_change:+.1f}%)"
                    ax.set_title(title, fontsize=8, fontweight='bold', color=color_title)
                    ...
                    ax.text(0.5, 0.5, f"⚠️ Error", ha='center', va='center', fontsize=7, color='orange')
                    ax.set_title(ticker.replace('.JK', ''), fontsize=8, color='orange')
                    ax.axis('off')
...
# --- 5. FOOTER ---
st.divider()
st.markdown("<div style='text-align: center; color: gray; font-size: 11px;'>📈 IHSG Chart Generator - Data dari Yahoo Finance</div>", unsafe_allow_html=True)
