...
# --- SETTING PAGE ---
st.set_page_config(page_title="Real-Time Screener", layout="wide")

@st.cache_data(ttl=3600)
def get_ticker_list():
    # Contoh list, pastikan gunakan .JK
...
def fetch_and_process(ticker):
    try:
        # 1. DOWNLOAD DATA (Gunakan auto_adjust=False agar harga Close tidak dimanipulasi)
        # Ambil data 7 hari untuk memastikan kita punya cukup data T-1 (kemarin)
        df = yf.download(ticker, period="7d", interval="1d", progress=False, auto_adjust=False)
        
        if df.empty or len(df) < 2:
            return None

        # 2. FIX MULTI-INDEX (Fitur yfinance terbaru yang bikin data 'ngaco')
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)

        # 3. AMBIL DATA DENGAN PRESISE
        # .iloc[-1] adalah baris terakhir (Hari ini/Running)
        # .iloc[-2] adalah baris sebelum terakhir (Kemarin/Tutup Sebelumnya)
        
        current_price = float(df['Close'].iloc[-1])
        prev_close = float(df['Close'].iloc[-2])
        volume_today = float(df['Volume'].iloc[-1])
        
        # Hitung rata-rata volume (20 hari terakhir untuk RVOL yang akurat)
        # Kita ambil data 20 hari khusus untuk ini
        hist_20d = yf.download(ticker, period="20d", interval="1d", progress=False, auto_adjust=False)
        if isinstance(hist_20d.columns, pd.MultiIndex):
            hist_20d.columns = hist_20d.columns.get_level_values(0)
        
        avg_vol_20d = float(hist_20d['Volume'].mean())

        # 4. KALKULASI METRIK
        change_pct = ((current_price - prev_close) / prev_close) * 100
        turnover = current_price * volume_today
        rvol = volume_today / avg_vol_20d if avg_vol_20d > 0 else 0

        return {
            "Ticker": ticker.replace(".JK", ""),
            "Price": current_price,
            "Prev Close": prev_close, # Tambahkan ini untuk verifikasi manual
...
# --- UI LOGIC ---
st.title("🚀 Precise Day Trading Screener")

st.sidebar.header("Filter")
min_turnover = st.sidebar.number_input("Min Turnover (IDR)", value=5_000_000_000)
...
if st.button("Jalankan Screening"):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(fetch_and_process, t): t for t in tickers}
    ...
    if results:
        ...
        # FILTERING
        final_df = main_df[main_df['Turnover'] >= min_turnover].sort_values(by="Change %", ascending=False)
