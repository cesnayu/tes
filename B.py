...
# --- KONFIGURASI HALAMAN ---
st.set_page_config(page_title="Professional Screener", layout="wide")

# 1. Cache List Saham (Gunakan .JK untuk BEI)
@st.cache_data(ttl=3600)
def get_ticker_list():
    # Contoh list, kamu bisa ganti dengan 800+ ticker kamu
...
def fetch_and_process(ticker):
    """Fungsi pengambil data per saham dengan proteksi error."""
    try:
        # Ambil data 5 hari terakhir (interval 1 hari) untuk hitung volume & price change
        # progress=False penting agar tidak mengotori console
        df = yf.download(ticker, period="5d", interval="1d", progress=False)
        
        if df.empty or len(df) < 2:
            return None

        # --- FIX MULTI-INDEX (Penyebab Error Ambiguity) ---
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)

        # Ambil data hari ini (T) dan kemarin (T-1)
        # Menggunakan .iloc untuk menghindari data bocor (leaks)
        last_price = float(df['Close'].iloc[-1])
        prev_close = float(df['Close'].iloc[-2])
        volume_today = float(df['Volume'].iloc[-1])
        avg_vol_5d = float(df['Volume'].mean())

        # Kalkulasi Metrik
        change_pct = ((last_price - prev_close) / prev_close) * 100
        turnover = last_price * volume_today
        rvol = volume_today / avg_vol_5d if avg_vol_5d > 0 else 0
...
min_val = st.sidebar.number_input("Min Turnover (IDR)", value=5_000_000_000, step=1_000_000_000)
min_gain = st.sidebar.slider("Min Gain %", 0.0, 15.0, 2.0)
min_rvol = st.sidebar.slider("Min RVOL (Vol Spike)", 0.5, 5.0, 1.5)
...
if st.button("Jalankan Screening"):
    ...
    # 2. Multi-threading dengan ThreadPoolExecutor
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        future_to_ticker = {executor.submit(fetch_and_process, t): t for t in tickers}
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_ticker)):
            ...
            status_text.text(f"Memproses {i+1}/{len(tickers)} saham...")

    # 3. Pengolahan DataFrame Akhir (Clean & Filter)
    if raw_results:
        # Ubah list of dict menjadi DataFrame
        main_df = pd.DataFrame(raw_results)

        # Pastikan tipe data numerik agar perbandingan (>=) tidak error
        main_df['Turnover'] = pd.to_numeric(main_df['Turnover'], errors='coerce')
        main_df['Change %'] = pd.to_numeric(main_df['Change %'], errors='coerce')
        main_df['RVOL'] = pd.to_numeric(main_df['RVOL'], errors='coerce')

        # Bersihkan data kosong (NaN)
        main_df = main_df.dropna()

        # Eksekusi Filter (Tanpa Ambiguity)
        # Kita menggunakan variabel boolean mask agar lebih stabil
        mask = (main_df['Turnover'] >= min_val) & \
               (main_df['Change %'] >= min_gain) & \
               (main_df['RVOL'] >= min_rvol)
        
        final_df = main_df[mask].sort_values(by="Change %", ascending=False)

        # Tampilkan Hasil
        st.subheader(f"Ditemukan {len(final_df)} Saham Sesuai Kriteria")
