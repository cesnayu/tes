...
def get_financial_dashboard(ticker_list):
    # 1. Setup Session untuk menghindari blokir (Prevent "Data Leaks"/Error)
    ...
    for ticker_symbol in ticker_list:
        try:
            print(f"Sedang mengambil data: {ticker_symbol}...")
            stock = yf.Ticker(ticker_symbol, session=session)
            
            # 2. Ambil Laporan Keuangan (Financials)
            # .financials biasanya memberikan data 4 tahun terakhir
            df_financials = stock.financials
            
            if df_financials.empty or 'Net Income' not in df_financials.index:
                print(f"  [!] Data Net Income tidak tersedia untuk {ticker_symbol}")
                continue

            # Ambil 3 periode terbaru
            net_income_series = df_financials.loc['Net Income'].head(3)

            for date, income in net_income_series.items():
                report_date = date.strftime('%Y-%m-%d')
                
                # 3. Ambil Harga Saham pada tanggal laporan tersebut
                # Kita ambil range 5 hari untuk memastikan market buka (bukan libur/weekend)
                history = stock.history(start=date, periods=5)
                ...
                final_results.append({
                    ...
                    'Net Income': income,
                    'Stock Price (Close)': round(close_price, 2) if close_price else "N/A"
                })

            # 4. Jeda acak (Anti-Bot) agar tidak kena Rate Limit
            time.sleep(random.uniform(1.5, 3.0))

        except Exception as e:
            print(f"  [Error] Terjadi kendala pada {ticker_symbol}: {e}")
...
# --- EKSEKUSI ---
# Gunakan akhiran .JK untuk saham di Bursa Efek Indonesia (BEI)
...
df_dashboard = get_financial_dashboard(daftar_saham)

# Tampilkan Hasil
print("\n--- Dashboard Net Income 3 Tahun Terakhir ---")
if not df_dashboard.empty:
    # Mengurutkan berdasarkan Ticker dan Tahun
    df_dashboard = df_dashboard.sort_values(by=['Ticker', 'Tahun'], ascending=[True, False])
    print(df_dashboard.to_string(index=False))
    
    # Simpan ke Excel/CSV jika perlu
    # df_dashboard.to_csv('data_saham_3thn.csv', index=False)
else:
