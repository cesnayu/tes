...
def get_optimized_dashboard(ticker_list):
    session = requests.Session()
    session.headers.update({'User-Agent': 'Mozilla/5.0'})
    
    final_results = []
    
    # Filter list agar tidak terlalu panjang untuk testing awal
    # ticker_list = ticker_list[:20] # Buka komen ini jika ingin tes 20 saham dulu

    print(f"Memulai proses untuk {len(ticker_list)} saham...")

    for i, ticker_symbol in enumerate(ticker_list):
        try:
            # Progress bar sederhana
            if i % 10 == 0:
                print(f"Progress: {i}/{len(ticker_list)} saham diproses...")

            stock = yf.Ticker(ticker_symbol, session=session)
            
            # Ambil Financials
            df_fin = stock.financials
            if df_fin.empty or 'Net Income' not in df_fin.index:
                continue

            # Ambil 3 tahun terakhir
            net_income = df_fin.loc['Net Income'].head(3)

            for date, income in net_income.items():
                # Ambil harga pada tanggal tersebut (langsung tanpa .history panjang)
                # Agar cepat, kita simpan info income-nya saja dulu
            ...
            # Jeda sangat singkat agar tidak terlalu lama tapi tetap aman
            time.sleep(0.5) 
...
# --- EKSEKUSI ---
# Cobalah dengan subset kecil dulu untuk memastikan tidak "hang"
...
] # Masukkan list kamu di sini

df_hasil = get_optimized_dashboard(daftar_saham_full)

if not df_hasil.empty:
    print("\n--- HASIL BERHASIL DIAMBIL ---")
    print(df_hasil.head(20))
    # Simpan ke Excel karena datanya banyak, lebih enak dilihat di Excel
    df_hasil.to_excel("hasil_net_income.xlsx", index=False)
    print("\nFile disimpan sebagai: hasil_net_income.xlsx")
else:
    print("Gagal mengambil data. Coba kurangi jumlah list sahamnya.")
