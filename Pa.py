...
st.title("Net Income Stock Screener")
...
def get_net_income(ticker):
    ...
    try:
        ...
        if "Net Income" not in income.columns:
            return None

        net = income["Net Income"].dropna()

        # ambil maksimal 3 tahun terakhir
        net = net.head(3)

        df = net.reset_index()

        df.columns = ["Year","Net Income"]
...
total_pages = math.ceil(len(stock_list) / stocks_per_page)
...
stocks_page = stock_list[start:end]

st.write(f"Showing {len(stocks_page)} stocks")
...
for i, ticker in enumerate(stocks_page):

    df = get_net_income(ticker)
