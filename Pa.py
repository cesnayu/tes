...
tab1, tab2, tab3 = st.tabs(["Price", "Fundamental", "Compare"])
...
with tab2:
    ...
    ticker_f = st.text_input("Ticker Fundamental", "BBCA.JK")

    stock = yf.Ticker(ticker_f)

    income = stock.financials.T

    net_income = income["Net Income"].head(3)

    df_income = net_income.reset_index()

    df_income.columns = ["Year", "Net Income"]

    st.subheader("Net Income (3 Years)")
